...
- 剪贴板快捷操作：复制内容为链接时显示「打开链接」（系统默认浏览器）；若已安装则额外显示用 Chrome / Edge / Firefox 打开；链接相关操作使用对应浏览器真实图标，样式与主面板应用图标一致；链接类操作优先于翻译 / 待办等通用文本操作。
- 主面板搜索时固定应用后图钉图标即时刷新；内置插件与第三方插件均可固定到「已固定」；固定后仍保留在「最近使用」且不改其顺序；「已固定」按固定时间与上次使用的较新者排序（手动固定或再次打开都会提到最前）。
- 主面板第三方插件图标与应用图标采用相同样式（圆角、背景、描边等），保留左上角「插件」角标；内置插件图标样式不变。
- 主面板禁用 WebView 原生右键菜单，改为独立浮动窗口菜单；应用/插件支持右键「打开」「打开位置」「固定」；最近使用支持「移出列表」；内置插件「打开位置」禁用；菜单为不透明窗口，不主动设置原生/H5 圆角（macOS 仍交由系统）；Windows 上预创建菜单窗口，避免同步创建导致死锁。
- 开发环境下打开/激活 WebView 控制台时，主面板不会因失焦自动关闭（Windows 按 `msedgewebview2` 进程窗口检测）；控制台仅在失焦时检测，系统外观由 Rust 推送 `os:appearance-changed`，避免前端定时轮询刷屏。
- 插件独立窗口在 Dock / 任务栏使用 Tempo 平台图标，右下角叠加插件图标角标；未配置图标时角标与主面板缺省插件图标一致。
- 插件开发助手：验证 MCP Tool 时可在大编辑器中编辑输入 JSON；按 `inputSchema` 预填各字段的空 / 默认值后作为 arguments 运行。
...
- 插件开发助手：Command / MCP「验证」对话框内容过长时可滚动；验证 Command 时可输入字符串 / 图片 / 文件参数，不再展示只读元数据。
