...
- 插件 SDK（破坏性变更）：`@tempo/sdk` 升级到 1.0，UI 改用 `connect()` 获取上下文非空的 Client，Runtime 改用 `defineRuntime()` 统一 setup 与逆序清理；Hybrid 可用一份 IPC 契约约束两侧频道、参数和返回值。
...
- 插件模板：同步 UI / Hybrid / Headless 模板至 2.0.7 和 Host API 2.1.0，并提供内置模板回退；Hybrid 的 UI、Runtime tsconfig 位于根目录，由 references 和 `tsc -b` 统一检查，同时通过共享 IPC 契约保持两侧类型一致。
