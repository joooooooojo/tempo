...
- 插件运行时（破坏性变更）：后台统一迁移到 Deno 2.9.6，不再回退 Node；插件需使用 Manifest v2 / Host API 2.0.0，旧插件须提升版本、重新打包并确认信任。Node/npm/Vite 继续用于构建，支持兼容的自包含 ESM 产物。
- 插件权限：默认拒绝额外文件、网络和环境访问，按 Manifest 声明授予数据目录、网络端点、环境变量及通知等 Host API 权限；升级前展示新旧权限，撤销信任会停止运行并关闭插件窗口。后台权限不替代 WebView 网络策略或 OS 级隔离，不开放子进程与原生扩展。
- 插件模板：同步 UI / Hybrid / Headless 模板至 2.0.2，并提供内置模板回退；Hybrid 的 UI、Runtime tsconfig 移至根目录，由根 tsconfig 的 references 和 `tsc -b` 统一检查，同时保持两侧类型隔离。
...
- 演示插件：Hello 2.0.1 补齐日志写权限，Welcome 0.2.0 更新 v2 Schema 并移除多余权限。
- 快捷键（Windows）：调整 Alt 事件转发，避免 `Alt+空格` 被后续键盘钩子误判为双击 Alt；保留组合键防抢占能力，并配对处理松开事件、抑制长按重复触发。
- 主面板：右键打开应用菜单时不再误关面板。
