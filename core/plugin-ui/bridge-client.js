...
(() => {
  ...
  const files = {
    async readText(path) {
      const result = await call("files.readText", { path });
      return new TextDecoder().decode(base64ToBytes(result?.base64));
    },
    writeText: (path, content) =>
      call("files.writeText", {
        path,
        base64: bytesToBase64(new TextEncoder().encode(String(content))),
      }).then(() => undefined),
    async readBytes(path) {
      const result = await call("files.readBytes", { path });
      return base64ToBytes(result?.base64);
    },
    writeBytes: (path, bytes) =>
      call("files.writeBytes", { path, base64: bytesToBase64(bytes) }).then(
        () => undefined,
      ),
    async list(path = "") {
      const result = await call("files.list", { path });
      return Array.isArray(result?.entries) ? result.entries : [];
    },
    async stat(path) {
      const result = await call("files.stat", { path });
      return result?.stat ?? null;
    },
    mkdir: (path, options = {}) =>
      call("files.mkdir", { path, recursive: options.recursive === true }).then(
        () => undefined,
      ),
    remove: (path, options = {}) =>
      call("files.remove", { path, recursive: options.recursive === true }).then(
        () => undefined,
      ),
    rename: (from, to) =>
      call("files.rename", { from, to }).then(() => undefined),
  };
