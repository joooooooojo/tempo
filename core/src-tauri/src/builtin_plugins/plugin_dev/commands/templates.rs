...
#[cfg(test)]
const BUNDLED_TEMPLATE_VERSION: &str = "2.0.6";
static BUNDLED_DENO_TEMPLATES: include_dir::Dir<'_> =
    include_dir::include_dir!("$CARGO_MANIFEST_DIR/../../docs/public/plugin-assets/releases/2.0.6");
    ...
    async fn downloads_and_scaffolds_the_published_hybrid_template() {
        ...
        let package: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(root.join("package.json")).unwrap())
                .unwrap();
        assert_eq!(package["dependencies"]["@tempo/sdk"], "^0.1.0");
