...
- **要做**：注册命令 / 应用、面板生命周期、主题、插件私有存储  
- **不做**：把读盘、HTTP、进程、注册表等封装成完整 OS API 面；**也不**把待办 / 短语等官方应用业务接口暴露给第三方插件  
...
```text
Plugin Package
├── manifest.json          # 元数据、贡献点、运行时入口、引擎版本（包最外层）
├── index.html             # 有 UI 必填：面板入口（与 manifest 同级）
├── main.mjs / main.js     # 无 UI 必填（混合可选）：Runtime 入口（与 manifest 同级；不用 index.js 以免与 UI 撞名）
├── icons/
└── README.md
```
...
1. **有 UI**（贡献 `apps[]`）：必须存在根级 `index.html`，且 `apps[].entry` 只能是 `index.html`
2. **无 UI**（headless）：必须存在根级 `main.js` 或 `main.mjs`，且 `main` 只能是二者之一
3. `manifest.json`、`index.html`、`main.mjs`/`main.js` 必须在同一级、包最外层；禁止 `dist/ui/...` 这类嵌套入口。UI 侧脚本可用 `index.js` 等任意资源名，由 `index.html` 自行引用。
...
| main 格式 | 包根级单一 bundled 文件：`main.mjs` 或 `main.js`；入口不得引用包外文件 |
...
```jsonc
{
  ...
  "license": "MIT",
  "categories": ["tools"],
  "main": "main.mjs",                 // 包根级 Runtime 入口：仅 main.mjs / main.js
    ...
    "apps": [
      {
        ...
        "keywords": ["demo", "示例"],
        "icon": "icons/app.png",
        "entry": "index.html",          // 有 UI 时固定为包根 index.html
...
| 官方应用业务数据（待办 / 短语等） | **不**经 Host Bridge 向第三方插件开放；插件用 `storage.plugin.*` 或 Runtime 自管数据 |
...
- `main` 只能是包根 `main.mjs` / `main.js`；有 UI 时 `apps[].entry` 只能是包根 `index.html`；`icon`、`executables[]` 只能是插件根目录内的规范化相对路径。不接受 URL、绝对路径、盘符、UNC 或符号链接跳转；Unix 只为 `executables[]` 中的普通文件设置执行位。
...
1. **`host.*`（Bridge）**：面板控制、主题、打开其它 App、插件私有存储等集成面。
...
SDK 统一封装三者。**实现上**宿主在每个插件 HTML 中自动注入 `__tempo__/client.js`，挂载 `window.plugin`：`plugin.invoke(name, params)` **只**调 Runtime，`plugin.host(name, params)` **只**调宿主，避免 command id 与宿主方法重名；另有 `on` / `ready`。底层仍走同一条实例绑定 Bridge，但入口已分命名空间。Gateway 不接受页面在 payload 中自报身份。内置 React 应用不经过此通道。
...
**明确不提供（也不需要）：** 完整 `fs.*` / `http.fetch` 代理 / `shell.exec` 官方封装——这些在每插件 Runtime 中用 Node 即可。内置待办 / 短语等应用是 Tempo 官方能力，**不**作为第三方插件 Host API 暴露。
    ...
    packages/
      com.example.polyglot/
        1.0.0/               # 安装后只读；current version 由 SQLite 指向
          manifest.json
          index.html         # 有 UI 时
          main.mjs           # 有 Runtime 时（或 main.js）
          icons/...
    data/
...
- [x] 内置 App 会话持久化  
- [x] `BuiltinApp` → `TempoApp`（含 `plugin-webview` 联合类型；见 `src/apps/types.ts`）  
- [x] `OpenAppOptions.params` 通用化（`resolveOpenAppParams`，见 `src/apps/types.ts`）  

### Phase 1 — MVP（本地插件 + 完整权限运行时）

- [x] manifest v1 校验（类型 / 长度 / 路径 / 贡献点引用）与声明式贡献事务（`manifest.rs`；单个插件解析或校验失败即整体跳过，不留半注册状态）  
- [x] staging 安全解包、全包 hash、版本目录、原子安装 / 更新回滚（`package.rs` + `trust::record_installed_version` 的 `pending_version` 分期 + `promote_plugin_pending_version`；启用中插件导入新版本会先挂起，用户信任后一键切换）  
- [x] **插件运行时**：检测 / 按需下载 / 校验 SHA-256 / 安装到 app_data；设置页「安装 / 卸载插件运行时」；纯 UI 包不强制（`runtime.rs`）  
- [x] Rust Supervisor 使用已安装的插件专用 Node，为每插件管理独立 Runtime 与进程树（`supervisor.rs`）  
- [x] 插件管理页：按包 hash 信任、Runtime 状态与错误、待切换版本；**开发目录导入与其未验证标识未实现**（见 Phase 2）  
- [x] UI：每插件资源 origin、最低 CSP、secure context、Esc 键盘转发、iframe `sandbox`（**架构调整为 iframe + `tempo-plugin://` 协议**，见 §14 附注；未采用独立进程的 raw Wry 子视图）  
- [x] 认证 UI Gateway + Runtime 专用本地 IPC：握手、帧协议、超时、取消、限流、结构化错误、main→UI 事件通道（`bridge.rs` + `supervisor.rs`；UI Gateway 经 `plugin_bridge_invoke` 校验而非 Wry 专用 handler，见 §14 附注）  
- [x] 声明式 App / Action 先注册，command / UI runtime 调用时懒激活（`loader.rs` + `supervisor.rs::ensure_started`）  
- [x] Headless actions + commands（可调 Node fs/net/process，见 `examples/plugins/com.example.hello`）  
- [x] 插件会话序列化 / 恢复，不保活 WebView（`ui.rs::save_session/load_session` + `plugin_ui_serialize_session`，300ms 预算）  
- [x] 示例插件：证明「非官方业务也能做」（`examples/plugins/com.example.hello`，含 README）  
- [x] 安装、UI IPC、命名空间负向测试（`package.rs` 路径穿越 + 符号链接拒绝、`bridge.rs` UI-only / 内部错误脱敏 / 载荷超限、`ids.rs` 非法 ID）；**进程清理暂无自动化测试**（`supervisor.rs::kill_process_tree` 仅手动/集成验证）  

### Phase 2 — 平台化

- [x] hooks（`hooks.rs::dispatch_event`，已接入 `clipboard.changed`；**schedules/cron 未实现**，留待后续）  
- [x] MCP 动态挂载 + 暴露开关：`plugin_mcp_exposure` 表 + `set_plugin_mcp_exposed` / `list_plugin_mcp_tools` 命令 + 设置页开关；MCP 侧新增 `tempo_list_exposed_plugin_tools` / `tempo_call_plugin_tool` 两个 meta-tool（`mcp_bridge.rs` + `mcp/server.rs`）——**实用主义实现**：不为每个插件工具动态注册独立 MCP tool，而是统一走这两个宿主 tool  
- [ ] 开发热重载、额外 activation events（本期新增的 `onStartup` 属于 Phase 1 §4.3 范围，非本期新增；开发目录热重载 / dev server entry 覆盖未实现）  
- [ ] 全包签名侧载、发布者公钥信任与密钥撤销  
...
| `src/pages/CommandPalettePage.tsx` | **已实现（架构调整）**：`PluginAppHost` 通过 `tempo-plugin://` 协议在**沙箱 `<iframe>`** 内加载插件 UI，而非 raw Wry 子视图（见下方附注） | — |
| `src-tauri/capabilities/default.json` | capability 按窗口覆盖 | 改为只匹配宿主 WebView label；插件 iframe 不获得任何 Tauri capability |
| `src-tauri/tauri.conf.json` | `withGlobalTauri=true`、全局 CSP 为空 | 关闭 `withGlobalTauri`；插件资源响应单独设置 CSP / origin |
| `src-tauri/src/plugins/package.rs` | **已实现**：安全解包、路径 / 符号链接校验、全包 hash、原子安装（更新回滚未做，见 §13） | — |
| `src-tauri/src/plugins/trust.rs` | **已实现**：包 hash 信任、启用前置校验；`plugin_mcp_exposure`（Phase 2）；开发目录确认与签名密钥信任未做 | — |
| `src-tauri/src/plugins/supervisor.rs` | **已实现**：每插件 Node Runtime、进程组清理、崩溃退避、状态机 | — |
| `src-tauri/src/plugins/bridge.rs` | **已实现**：ConnectionContext、RPC 信封、限流、超时、取消；UI Gateway 经 `plugin_bridge_invoke` + iframe postMessage（非 Wry 专用 IPC handler） | — |
| `src-tauri/src/plugins/ui.rs` | **已实现（架构调整）**：`tempo-plugin://` 资源协议、CSP、会话持久化，见下方附注 | — |
| `src-tauri/src/plugins/hooks.rs` | **已实现（Phase 2）**：`dispatch_event` 扫描 `contributes.hooks` 并 fire-and-forget 调用；已接入 `clipboard.changed` | schedules/cron 未做 |
| `src-tauri/src/plugins/mcp_bridge.rs` | **已实现（Phase 2）**：`tempo_list_exposed_plugin_tools` / `tempo_call_plugin_tool` 两个宿主 meta-tool 背后的暴露清单与调用桥接 | — |
| `plugin-runtime/bootstrap.mjs` | **已实现**：Node 启动器、Unix socket / named pipe transport、加载单一 main bundle | — |
| `src-tauri/src/mcp/` | **已实现（Phase 2）**：静态业务 tool 之外新增两个插件 meta-tool（动态挂载 + 暴露开关，见 §13） | — |
| SQLite migrations | **已实现**：`plugins`、`plugin_versions`、`plugin_sessions`、`plugin_storage`、`plugin_mcp_exposure`；`publisher_trust` 表结构已建但签名验证未接入 | — |
| 设置页 | **已实现**：插件管理 / 信任 / Runtime 诊断 / MCP 暴露开关；市场入口未做 | — |

> **架构附注（UI 承载方式）**：本实现未采用 §5 描述的「raw Wry 子 WebView（不注册 Tauri IPC）」，
> 而是让 `PluginAppHost` 用一个 `<iframe>` 加载 `tempo-plugin://{pluginHash}/...` 协议资源（与宿主
> 页面不同源），插件页与宿主用 `postMessage` 通信；宿主侧 `plugin_bridge_invoke` 在转发到
> `bridge::dispatch` 前会校验 `viewInstanceId` 与调用方 `pluginId` 是否匹配、`event.source`/`event.origin`
> 是否为该 iframe（见 `src/apps/PluginAppHost.tsx`），因此**数据边界仍由 Rust 侧的
> `ConnectionContext` 鉴权**（§0.4 第 3 类边界），未变。已生效的纵深防护：`withGlobalTauri=false`
> （`tauri.conf.json`）、不同源阻止 iframe 直接读写宿主 DOM/JS 对象。**已知与 §5 设计的差距**（未
> 逐项验收，留作后续加固）：①`<iframe>` 当前未设置 `sandbox` 属性；②未验证 Tauri 的 IPC
> 初始化脚本是否只注入顶层 frame 而不注入子 frame（若会注入，插件页可能仍拿到
> `window.__TAURI_INTERNALS__.invoke`，是比 raw Wry 更弱的边界）；③iframe 与宿主共享同一个
> 渲染进程 / `WebviewWindow`，不是独立操作系统进程，进程级崩溃隔离对插件 UI 不适用（Runtime/`main`
> 一侧不受影响，仍是独立 Node 子进程，隔离保证不变）。
...
```text
com.example.hello/
  manifest.json          # 与入口同级、包最外层
  main.mjs               # Runtime：activate + registerCommand（main.js 亦可）
  index.html             # UI 入口（有 apps[] 时必填）
  index.js               # UI 脚本（由 index.html 引用，可选资源）
  icons/app.png
```
