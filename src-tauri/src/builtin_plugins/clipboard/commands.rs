...
use crate::builtin_plugins::clipboard::watcher::prewarm_clipboard_image_cache;
use crate::builtin_plugins::clipboard::watcher::{
...
fn main_panel_seed_from_entry(
    ...
    if entry.kind == "file" {
        let paths = crate::builtin_plugins::clipboard::files::parse_clipboard_paths(&entry.content)?
            .into_iter()
...
pub fn get_main_panel_clipboard_seed(
    ...
    if recent.kind == "image" {
        ...
        let conn = state.db.lock();
        let Some(entry) = crate::builtin_plugins::clipboard::db::get_clipboard_entry(&conn, entry_id) else {
            return Ok(None);
        };
        drop(conn);
