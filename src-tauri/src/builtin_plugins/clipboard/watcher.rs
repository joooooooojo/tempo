...
use crate::builtin_plugins::clipboard::images::save_clipboard_image_png;
use crate::builtin_plugins::clipboard::images::{
...
// Note: left as a single module — capture (OS clipboard read / platform listeners)
// and persist (history insert + main-panel seed) are tightly interleaved in the poll
// loop, so a deep capture/persist split would mostly add churn without clear seams.
    ...
    crate::logging::spawn_named("tempo-clipboard-watcher", move || {
        ...
        loop {
            ...
            if last_retention_purge.elapsed() >= std::time::Duration::from_secs(3600) {
                ...
                let conn = state.db.lock();
                crate::builtin_plugins::clipboard::db::purge_clipboard_history_by_retention(&conn, &retention);
            }
...
fn baseline_clipboard_hashes(
    ...
    match read_clipboard_snapshot() {
        Ok(ClipboardSnapshot::Files(paths)) => {
            ...
            let hash = crate::builtin_plugins::clipboard::db::hash_content(&content);
        ...
        Ok(ClipboardSnapshot::Image(image)) => {
            ...
            if pixel_count == 0 || pixel_count > crate::builtin_plugins::clipboard::db::MAX_CLIPBOARD_IMAGE_PIXELS {
                return;
            }
            let Some(png_bytes) = encode_rgba_png(width, height, image.bytes.as_ref()) else {
                return;
            };
            if png_bytes.len() > crate::builtin_plugins::clipboard::db::MAX_CLIPBOARD_IMAGE_BYTES {
                return;
            }
        ...
        Ok(ClipboardSnapshot::Text(text)) => {
            ...
            let hash = crate::builtin_plugins::clipboard::db::hash_content(&text);
            last_text_hash.clear();
...
pub fn use_clipboard_entry(
    state: &AppState,
    app: &AppHandle,
    entry: &crate::builtin_plugins::clipboard::db::ClipboardEntry,
) -> Result<(), String> {
    let settings = {
        let conn = state.db.lock();
        load_settings(&conn)
...
fn clipboard_reference_kind(content: &str) -> &'static str {
    if crate::builtin_plugins::clipboard::images::is_clipboard_image_storage_key(content) {
        "storage-key"
    } else {
        "external-reference"
    }
